...
class Atomic_Events {
	...
	public function __construct() {
		if (defined('ATOMIC_EVENTS_VERSION')) {
			$this->version = ATOMIC_EVENTS_VERSION;
		} else {
			$this->version = '1.0.0';
		}
		$this->plugin_name = 'atomic-events';
	...
	private function load_dependencies() {
		...
		// utils
		require ATOMIC_EVENTS_PLUGIN_DIR . 'utils/class-template-loader.php';
		require ATOMIC_EVENTS_PLUGIN_DIR . 'utils/class-atmc-template-loader.php';
		require ATOMIC_EVENTS_PLUGIN_DIR . 'utils/class-atomic-event-date-utils.php';
		require ATOMIC_EVENTS_PLUGIN_DIR . 'utils/class-atomic-template-utils.php';
		require ATOMIC_EVENTS_PLUGIN_DIR . 'utils/class-atomic-member-utils.php';
		require ATOMIC_EVENTS_PLUGIN_DIR . 'utils/class-atomic-events-image.php';
		require ATOMIC_EVENTS_PLUGIN_DIR . 'utils/class-atomic-event-link.php';
		require ATOMIC_EVENTS_PLUGIN_DIR . 'utils/class-atomic-image.php';
		...
		$this->loader = new Atomic_Events_Loader();
	}

	/**
	 * Define the locale for this plugin for internationalization.
	 *
	 * Uses the Atomic_Events_i18n class in order to set the domain and to register the hook
	 * with WordPress.
	 *
	 * @since    1.0.0
	 * @access   private
	...
	private function set_locale() {
		...
		$plugin_i18n = new Atomic_Events_i18n();
	...
	private function define_admin_hooks() {
		...
		$plugin_admin = new Atomic_Events_Admin($this->get_plugin_name(), $this->get_version());

		$this->loader->add_action('admin_enqueue_scripts', $plugin_admin, 'enqueue_styles');
		$this->loader->add_action('admin_enqueue_scripts', $plugin_admin, 'enqueue_scripts');

		$evet_post_type = new AtomicEvents_PostType($this->get_version());
		$this->loader->add_action('init', $evet_post_type, 'create_event_post_type');

		$evet_taxonomy = new AtomicEventTags_Taxonomy($this->get_version());
		$this->loader->add_action('init', $evet_taxonomy, 'register_taxonomy');

		/**
		 * Template
		 */
		$tpl_util = new AtomicEvent_TemplateUtils();
	...
	private function define_public_hooks() {
		...
		$plugin_public = new Atomic_Events_Public($this->get_plugin_name(), $this->get_version());
		...
		$shortcodes = new AtomicEvents_Shortcode();
	 ...
	 *
	 * @since     1.0.0
	 * @return    Atomic_Events_Loader    Orchestrates the hooks of the plugin.
	 */
